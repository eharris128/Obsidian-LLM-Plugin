...
Backends (both in `src/Whisper/WhisperService.ts`): `"openai"` (`/audio/transcriptions`, whisper-1, uses `openAIAPIKey`) and `"sidecar"` (local Python `whisper-server.py`, faster-whisper; uploads via `requestUrl` with a hand-built multipart body — `encodeMultipartFormData()` in `WhisperService.ts` — because `requestUrl` accepts neither `FormData` nor streaming and the community review flags raw `fetch`). The 60 s timeout is a `Promise.race` — `requestUrl` cannot abort the underlying request.
